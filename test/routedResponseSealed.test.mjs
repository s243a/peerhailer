...
test("4b. F1 (documented intent): a legacy sealed body of exactly {responseSealKey, body} with a valid X25519 key is header-stripped", () => {
  const a = machine("alice");
  const b = machine("bob");
  const c = machine("carol");
  // A legacy origin (or an embedder calling wrap with recipientKey but no responseSealKey)
  // whose body *is* exactly the reserved two-key shape with a VALID X25519 PEM is read as a
  // header: the consumer gets the inner body, and the reply would seal to the *embedded* key
  // (carol's here), not the origin's. That is the cost of the reserved-`__routed*`-key
  // discriminator, pinned here as tested INTENT — the clean fix (a distinct manifest
  // payloadMode) is deferred to the next manifest bump. The origin's signature still
  // authenticates the whole plaintext, so the destination is fooled only about *where to
  // reply*, never about *who sent it*. (Test 4 pins the sibling invalid-key case → response-key.)
  const legacyBody = { [ROUTED_RESPONSE_SEAL_KEY_FIELD]: c.identity.sealPublicKey, body: { hello: "inner" } };
  const opened = openAt(wrapSealed(a, b, legacyBody), b); // no responseSealKey → wrap adds no header of its own
  assert.equal(opened.ok, true);
  assert.deepEqual(opened.body, { hello: "inner" }, "the inner body is surfaced, not the two-key object");
  assert.equal(opened.responseSealKey, c.identity.sealPublicKey, "the embedded key is taken as the response key (the F1 ambiguity)");
});
