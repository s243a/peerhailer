...
const wrap = (origin, dest, body, over = {}) =>
  wrapRoutedMessage({
    ...
    body,
    messageId: "_9_dyjXkLPnraDMak3Jg0w", // 16+ bytes base64url
    now: T,
    validityMs: 60_000,
    ...over,
  });
...
test("a wrapper built for this destination round-trips, exposing the body and origin", () => {
  ...
  const opened = openRoutedMessage(w, { selfKeyId: b.keyId, guard: guardAt() });
...
test("the same wrapper delivers once; a replay through a fresh open is refused", () => {
  ...
  const guard = guardAt();
  assert.equal(openRoutedMessage(w, { selfKeyId: b.keyId, guard }).ok, true);
  assert.deepEqual(openRoutedMessage(w, { selfKeyId: b.keyId, guard }), { ok: false, reason: "replay:duplicate" });
});
...
test("a wrapper addressed to someone else is refused by the destination self-check", () => {
  ...
  assert.deepEqual(openRoutedMessage(w, { selfKeyId: c.keyId, guard: guardAt() }), { ok: false, reason: "not-for-me" });
});
...
test("a swapped payload breaks the digest binding", () => {
  ...
  assert.deepEqual(openRoutedMessage(tampered, { selfKeyId: b.keyId, guard: guardAt() }), { ok: false, reason: "payload-digest" });
});
...
test("any change to a signed manifest field breaks the signature", () => {
  ...
  for (const [field, value] of [
    ...
    const opened = openRoutedMessage(
      { ...w, manifest: { ...w.manifest, [field]: value } },
      { selfKeyId: b.keyId, guard: guardAt() },
    );
    assert.equal(opened.ok, false, `${field} tamper should be refused`);
  }
});

test("re-signing a tampered manifest with a stranger's key still fails the origin binding", () => {
  const a = machine("alice");
  const b = machine("bob");
  const mallory = machine("mallory");
  // Mallory intercepts, rewrites the destination, and re-signs with her own key —
  // but the manifest still names alice's originKeyId, and her attached record is hers.
  const w = wrap(a, b, { secret: true });
  const forged = wrapRoutedMessage({
    self: mallory.self,
    privateKey: mallory.id.privateKey,
  ...
  // Mallory's own wrapper is internally consistent (origin = mallory) — it opens as
  // FROM mallory, not from alice. That is the point: authenticated to *her* key.
  const opened = openRoutedMessage(forged, { selfKeyId: b.keyId, guard: guardAt() });
  assert.deepEqual(opened, { ok: true, body: { secret: true }, originKeyId: mallory.keyId });
  assert.notEqual(opened.originKeyId, a.keyId);
});
...
test("attaching a different peer's record than the manifest names is refused", () => {
  ...
  assert.deepEqual(openRoutedMessage(swapped, { selfKeyId: b.keyId, guard: guardAt() }), { ok: false, reason: "origin-mismatch" });
});
...
test("an out-of-window envelope is refused before it is reserved", () => {
  ...
  assert.deepEqual(openRoutedMessage(w, { selfKeyId: b.keyId, guard: late }), { ok: false, reason: "replay:expired" });
});
...
test("a signed but non-clear payloadMode is refused, not reinterpreted as cleartext", () => {
  ...
  assert.deepEqual(openRoutedMessage(w, { selfKeyId: b.keyId, guard: guardAt() }), { ok: false, reason: "unsupported-mode" });
});
...
test("a multi-block manifest is refused — a fragment is not delivered as a whole body", () => {
  ...
  assert.deepEqual(openRoutedMessage(w, { selfKeyId: b.keyId, guard: guardAt() }), { ok: false, reason: "multi-block" });
});
...
test("a body that fails to parse is refused and burns no replay reservation", () => {
  ...
  const guard = guardAt();
  assert.deepEqual(openRoutedMessage(w, { selfKeyId: b.keyId, guard }), { ok: false, reason: "body" });
...
test("malformed wrappers are refusals, not throws", () => {
  ...
  for (const bad of [null, undefined, 42, "x", {}, { manifest: {}, originRecord: null, payload: "" }]) {
    const opened = openRoutedMessage(bad, { selfKeyId: b.keyId, guard: g });
