...
- The threaded response is now **signed and sealed**. `delivered`, acknowledgements, and
  ...
  - **Known limits on the response seal (external review, LOW).** (F1) The response-key header is
    discriminated *in band* by its exact two-key `{__routedResponseSealKey, body}` shape. A **legacy
    origin or embedder** whose own sealed body is exactly that shape with a valid X25519 key is
    misparsed as a header: the consumer gets the inner body and the reply seals to the *embedded*
    key. It is a confused deputy about *where to reply*, never about *who sent* (the origin's
    signature authenticates the whole plaintext); it needs the origin's app to author that exact
    reserved shape plus a return-path relay, and no *content* is ever accepted in the clear. Pinned
    as tested intent (test `4b`); the clean fix is a distinct manifest `payloadMode`, folded into the
    reserved `originSealKeyId` bump above. (F2) A relay can inject the public refusal shape
    `{refused:true,reason}` onto a *delivered* sealed reply, relabeling it a refusal
    (`responseSeal:none`, sealed field skipped) — availability/labeling only: no content is accepted
    and the verified `delivered` receipt contradicts the printed `refused`, so the contradiction is
    visible. This is the "threaded response is malleable feedback; the receipt is the proof" residual,
    now including the sealed-field-skip interaction. (F3) `__routedSealedResponse` is a reserved
    response-field name (like `__routedRecord`/`__routedReceipt`): a clear-path consumer body carrying
    that exact field has it dropped.
